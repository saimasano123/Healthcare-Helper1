        ...
        {chatResponse ? (
          <div className="chat-response">
            ...
            {chatResponse.entities && Object.keys(chatResponse.entities).length > 0 && (
              <div>
                <h4>Entities:</h4>
                <pre>{JSON.stringify(chatResponse.entities, null, 2)}</pre>
              </div>
            )}
            {chatResponse.recommendations && chatResponse.recommendations.length > 0 && (
              <div>
                ...
                <ul>
                  {chatResponse.recommendations.map((rec, idx) => (
                    <li key={idx}>{rec}</li>
                  ))}
                </ul>
              </div>
            )}
            {chatResponse.coverage && chatResponse.coverage.length > 0 && (
              <div>
                <h4>Coverage:</h4>
                <pre>{JSON.stringify(chatResponse.coverage, null, 2)}</pre>
              </div>
            )}
            {chatResponse.extracted_text && (
              <div>
                <h4>Extracted Text:</h4>
                <pre>{chatResponse.extracted_text}</pre>
              </div>
            )}
            {chatResponse.confidence !== undefined && (
              <div>
                <strong>Confidence:</strong> {chatResponse.confidence}
              </div>
            )}
          </div>
        ) : (
